...
public class AddToCartController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            int quantity = 1;
                for(CartDTO item : cart.getItems(user_id)){
                    if(item.getProd_id().equals(prod_id) ){
                        cart.updateItems(user_id, prod_id, item.getQuantity() + quantity);
                        check = false;
                ...
                if(check){
                    cart.addItem(user_id, prod_id, quantity);
                    session.setAttribute("cart", cart);
                url = "shop.jsp";
                request.setAttribute("msg", "Add to Cart successfully");
                }
